  ...
  return (
    knex.schema
      ...
      .createTable("alerts", table => {
        ...
        table.integer("alerts_type_id").notNullable().references("id").inTable("alerts_type").onUpdate("CASCADE").onDelete("RESTRICT");
      })
      //-----------------------------------------------------------------------------------------------
      //GRANTS TABLE
      ...
      .createTable("grants_modification_history", table => {
        table.increments("id"),
        table.integer("grant_id").notNullable().references("id").inTable("grants").onUpdate("CASCADE").onDelete("RESTRICT"),
      ...
      .createTable("category_grants", table => {
        table.increments("id"),
        table.integer("grants_id").notNullable().references("id").inTable("grants").onDelete("RESTRICT"),
        table.integer("category_id").notNullable().references("id").inTable("category_keys").onDelete("RESTRICT");
      })
      //-----------------------------------------------------------------------------------------------
      //ELEGIBILITY TABLE
      ...
      .createTable("elegibility_grants", table => {
        table.increments("id"),
        table.integer("grants_id").notNullable().references("id").inTable("grants").onDelete("RESTRICT").onUpdate("CASCADE"),
        table.integer("elegibility_id").notNullable().references("id").inTable("elegibility").onUpdate("CASCADE").onDelete("RESTRICT");
      })
      //-----------------------------------------------------------------------------------------------
      //SAVED GRANTS
      ...
      .createTable("application_status", table => {
        table.increments("id"),
        table.string("status_name", 15).notNullable().unique();
      ...
      .createTable("grant_applications", table => {
        table.increments("id"),
        table.integer("user_id").notNullable().references("id").inTable("users").onDelete("RESTRICT").onUpdate("CASCADE"),
        table.integer("grant_id").notNullable().references("id").inTable("grants").onUpdate("CASCADE"),
        ...
        table.timestamp("created_at").notNullable(),
        table.integer("status", 1).notNullable().references("id").inTable("application_status");
      ...
      .createTable("status_history", table => {
        table.increments("id"),
        table.integer("grants_id").notNullable().references("id").inTable("grant_applications").onUpdate("CASCADE").onDelete("RESTRICT");
        table.integer("status_id").notNullable().references("id").inTable("application_status").onUpdate("CASCADE").onDelete("RESTRICT"),
        table.timestamp("created_at").notNullable();
      })
      //----------------------------------------------------------------------------------------------
      //----------------------------------------------------------------------------------------------
      ...
      .createTable("cities", table => {
        table.increments("id"),
        table.integer("state_id").notNullable().references("id").inTable("states").onDelete("RESTRICT").onUpdate("CASCADE"),
        table.string("city_name", 100).notNullable();
      })
      //COUNTIES TABLE
      .createTable("counties", table => {
        table.increments("id"),
        table.integer("state_id").notNullable().references("id").inTable("states").onUpdate("CASCADE").onDelete("RESTRICT"),
        table.string("county_name", 100).notNullable();
      })
      .createTable("regions", table => {
        table.increments("id"),
        table.integer("grant_id").notNullable().references("id").inTable("grants").onDelete("RESTRICT").onUpdate("CASCADE"),
        table.boolean("country_wide").notNullable().defaultTo("false");
        table.integer("state_id").references("id").inTable("states").onDelete("RESTRICT").onUpdate("CASCADE"),
          //   table.integer('city_id').notNullable().references('id').inTable('cities')
          //        .onDelete('RESTRICT')
          //        .onUpdate('CASCADE'),
        table.integer("county_id").references("id").inTable("counties").onDelete("RESTRICT").onUpdate("CASCADE");
      })
      // CATEGORIES BY USERS INTERMEDIARY TABLE
      .createTable("category_user", table => {
        table.increments("id"),
        table.integer("user_id").notNullable().references("id").inTable("users");
        table.integer("category_id").notNullable().references("id").inTable("category_keys").onUpdate("CASCADE").onDelete("RESTRICT");
      })
      // ELIGIBILITY BY USERS
      .createTable("eligibility_user", table => {
        table.increments("id"),
        table.integer("user_id").notNullable().references("id").inTable("users");
        table.integer("eligibility_id").notNullable().references("id").inTable("elegibility").onUpdate("CASCADE").onDelete("RESTRICT");
      })
  );
};
